...
function findSystemProxyCheckbox(root: ReactTestRenderer) {
  const label = root.root.findAll((node) => (
    node.type === 'label'
...
describe('Sites detect race handling', () => {
  ...
  it('passes the current system proxy selection into manual detection', async () => {
    ...
    try {
      ...
      await act(async () => {
        findPrimarySiteUrlInput(root).props.onChange({ target: { value: 'https://proxied.example.com' } });
      });
      await act(async () => {
        findProxyUrlInput(root).props.onChange({ target: { value: 'http://127.0.0.1:7890' } });
      });
      await act(async () => {
