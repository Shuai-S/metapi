...
const zhToEn: Record<string, string> = {
  ...
  '控制台': 'Console',
  '功能': 'Tools',
  '卡密转 sub2api': 'Card Key to sub2api',
  '本地数据处理工具': 'Local data utilities',
  '将卡密批量导出文本转换为 sub2api 可导入的 JSON 文件': 'Convert card-key exports into sub2api-compatible JSON files',
  '选择卡密导出文件': 'Choose a card-key export',
  '支持 UTF-8 / UTF-8 BOM 的 .txt 文件': 'Supports UTF-8 and UTF-8 BOM .txt files',
  '严格模式': 'Strict mode',
  '遇到无效行时立即停止': 'Stop immediately on an invalid line',
  '未找到可转换的账号': 'No convertible accounts found',
  '文件转换失败': 'File conversion failed',
  ' 个输入账号': ' input accounts',
  ' 个输出账号': ' output accounts',
  ' 条提示': ' notices',
  '下载 sub2api JSON': 'Download sub2api JSON',
  'Session 转换器': 'Session Converter',
  '浏览器本地解析，不上传 Token，不写入存储': 'Processed locally in your browser. Tokens are not uploaded or stored.',
  '浏览器本地解析，Token 不上传、不写入存储；仅保存导入分组设置': 'Processed locally in your browser. Tokens are not uploaded or stored; only the import group setting is saved.',
  'ChatGPT Session 格式转换': 'ChatGPT Session Format Converter',
  '支持嵌套 JSON、逐行 JSON、卡密导出文本及多文件': 'Supports nested JSON, JSON Lines, card-key exports, and multiple files',
  'Sub2API 输出账号设置': 'Sub2API account output settings',
  '导入分组': 'Import group',
  '输入分组配置': 'Enter group configuration',
  '并发': 'Concurrency',
  '账号倍率': 'Account rate',
  '模型': 'Models',
  '移除模型': 'Remove model',
  '添加模型': 'Add model',
  '输入模型后按回车': 'Enter a model and press Enter',
  'Codex 指纹收敛': 'Codex fingerprint convergence',
  '关闭（透传，默认）': 'Off (passthrough, default)',
  '仅设备': 'Device only',
  '设备+会话': 'Device + Session',
  '完全收敛': 'Full convergence',
  '多人共享同一 OAuth 账号时，收敛设备和会话标识；部分账号开启后可能出现额度缩水。': 'When an OAuth account is shared, converge device and session identifiers. Some accounts may see reduced quota after enabling convergence.',
  '部分账号开启收敛后可能出现额度缩水': 'Some accounts may see reduced quota after enabling convergence',
  '输出格式': 'Output format',
  '导入后立即触发刷新 Token': 'Refresh token immediately after import',
  '输入数据': 'Input data',
  '个文件': 'files',
  '选择文件': 'Choose files',
  '填入示例': 'Load example',
  'Session JSON 输入': 'Session JSON input',
  '释放以读取文件': 'Drop files to read',
  '请选择 JSON 或 TXT 文件': 'Choose JSON or TXT files',
  '已忽略非 JSON/TXT 文件': 'Non-JSON/TXT files were ignored',
  '文件读取失败': 'Could not read the selected files',
  '识别': 'Detected',
  '转换': 'Converted',
  ...
  '转换输出': 'Converted output',
  '转换后将在此显示 JSON': 'Converted JSON will appear here',
  '输出已就绪': 'Output ready',
  '暂无输出': 'No output',
  '转换统计': 'Conversion summary',
  ' 个账号': ' accounts',
  ' 输出格式': ' output format',
  ' 个跳过项': ' skipped',
  '邮箱': 'Email',
  ...
  '来源': 'Source',
  '条': 'more',
  '未找到包含 accessToken 和账号标识的 Session 对象': 'No Session object containing an accessToken and account identity was found',
  '缺少 accessToken': 'Missing accessToken',
  '仪表盘': 'Dashboard',
  '站点': 'Sites',
  '账号': 'Accounts',
  '未关联站点': 'Unlinked Site',
